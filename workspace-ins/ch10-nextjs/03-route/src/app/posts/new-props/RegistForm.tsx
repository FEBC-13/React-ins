...
export default function RegistForm({createPost}: {createPost: (prevState: PostInfoRes | null, formData: FormData) => Promise<PostInfoRes>}) {
  ...
  return (
    <>
      ...
      {state?.ok===0 && <span>{state.errors?.[0]?.msg}</span>}
      {state?.ok===1 && <span>등록 완료</span>}
    </>
  )
}