...
function useAxiosInstance() {
  ...
  instance.interceptors.response.use((response) => {
    ...
    if(response.data?.ok !== undefined){ // 응답 데이터에 ok 프로퍼티가 있으면 0이나 1대신 boolean으로 변환
      response.data.ok = !!response.data.ok;
    }
    return response;
  }, (error) => {
    // 2xx 외의 범위에 있는 응답 상태 코드는 이 함수가 호출됨
    // 공통 에러 처리
    console.error('에러 응답 인터셉터 호출', error);
    
    toast.error('잠시후 다시 이용해 주시기 바랍니다.', {
      position: "top-center",
    ...
    // 호출하는 곳에 에러를 수정해서 전파(catch 블럭으로 전달)
    return Promise.reject(new Error('잠시후 다시 이용해 주시기 바랍니다.'));
  });
