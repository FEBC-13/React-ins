...
function App() {
  // "use no memo"; // React 컴파일러에게 이 컴포넌트를 최적화하지 말라고 명시적으로 지시
  ...
  // 조건부 Hook 사용시 리액트 규칙을 어기므로 컴파일러가 최적화 하지 않음
  if (quantity > 3) {
    let conditionalHook = useState('조건부 상태');
  }
  ...
  return (
    <>
      ...
      <div>
        ...
        수량: <input 
          type="number" min="1" max={ data.quantity - data.buyQuantity } value={ quantity } onChange={ handleQuantityChange } />
